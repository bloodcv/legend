...
verify_signature()
{
  # Verify secret
	...
	if [ "${expected}" = "${actual}" ] && [ $expire_time -ge `date +%s` ]
	then
...
expire_time=`echo "${payload}" | base64_decode | jq ".exp"`

verify_signature "${header}.${payload}" "${signature}" "${expire_time}"